...
from .prm_values import all_params, rta_prm_names, vehicle_prm_names, participant_prm_names, params_for_prediction
...
def prediction(request):
    return render(request, 'prediction.html', {'params_for_prediction': params_for_prediction})
    ...
    if not appear_keys:
        ...
        for i in range(len(appear_keys)):
            if appear_keys[i] in key_to_label.keys():
                appear_values[i] = labeler[key_to_label[appear_keys[i]]].transform(appear_values[i])

        df = pd.DataFrame(data=np.array(appear_values).T,
                          columns=appear_keys)
        df = df[['light', 'weather', 'experience', 'brand', 'color', 'gender', 'year']].astype(int)
        print(df)
